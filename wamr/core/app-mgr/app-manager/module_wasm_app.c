...
wasm_app_module_install(request_t * msg)
{
    ...
    /* Parse heap size */
    heap_size = CONFIG_DEFAULT_APP_HEAP_SIZE;
    ...
    switch (package_type) {
        ...
        case Wasm_Module_AoT:
        {
            ...
            /**
             * See https://github.com/bytecodealliance/wasm-micro-runtime/issues/352
             * this is a temporary fix to address the issue where secions were not
             * part of the raw .aot binary. instead of using wasm_runtime_load_from_sections
             * we directly load from the binary.
             *
             * Will update once that issue is answered/resulved on github.
             */
            char error_buf[128];
            ...
            inst = wasm_runtime_instantiate(module, CONFIG_DEFAULT_APP_STACK_SIZE, heap_size, err, err_size);
            if (!inst) {
                SEND_ERR_RESPONSE(msg->mid,
